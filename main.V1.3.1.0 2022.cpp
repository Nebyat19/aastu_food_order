/*
4/29/22------------------------------------------------------>date
This restaurant management system was made by AASTU second year students to manage
a renowned restaurant called A+ lounge in AASTU
*/
#include<iostream>
#include <stdlib.h>
using namespace std;
...
const int MAX_ORDER=500; // Maximum Order The Restaurant Can Take
const int TOTAL_FOOD=33; // Maximum Number Of Food That Can Be Available In The Restaurant
const int ORDER_COLUMN=4; // Order list Array Column Size
const int FOOD_COLUMN=2; // Food List And Ordered Food List Column Size
...
/* List Of Food Served Casually, Some Foods Can Be Canceled Or Added By Admin*/
{		"Name             ",	"Price"	},
{		"Enkulal Firfir   ",	"30"	},
{		"Dabo Firfir      ",	"20"	},
{		"Pasta Be Enkulal ",	"30"	},
...
firstPage();
system("pause"); // Used To Halt The Execution Of The Parent Cpp Program
    return 1;
}
 ...
 cout<<"\n\t**************************************************************************"<<endl;
   ...
   lastFoodIndex+=1;
	 	...
	 	food_list[lastFoodIndex][1]="     "+price;
	 		...
	 		cout<<	"\t\t Enter Any Number To Finish";
	 			char key;
	 				cin>>key;
	 					system("cls");
	 				firstPage();
...
loop1:
   ...
   cin >> rawr;
	if(rawr==1) {
		...
		totalOrder+=1;
		index+=1;
		  totalPrice+=amount*foodPrice(order);//accessing the food price from food list array
...
int conf;
 cout<<" \tYou have ordered:\n\n";
  ...
  for(int i=0;i<total_food;i++){//print all orders
  ...
  cout<<"                                  \t+==================+\n";
  cout<<"\tTotal Price: "<<total_price<<"\t\t\tPayment Code: 25KX1";
  cout<<"\n\tConfirm Your Order: Press Any Number to Confirm Except [0]: ";
  getTotalOrder();
  cin>>conf;
if(conf!=0){
/*  payment*/
...
cout<<"\t\t\t*******************************************************\n";
int key;
	cout<<"\tInput any Number to go back: ";
cin>>key;
system("cls");
optionPage('1');
}else{
	...
	for(int j=0; j<4; j++){
   		order_list[order_number][j]=0;//Making The List Empty
...
float price =atof(food_list[order][1].c_str()); //Changing A String Data Type To Float
	...
	return price;//Get Price From The Array And Return It As Float

}
string foodName(int order){
return food_list[order][0];	// Return Food Name From The Food List Array
}
int getTotalOrder(){ //Count The Total Order And Return The Last Index Of The Order List Array
	int order=0;
   for(int i=0; i<500;i++){
...
cout<<"\t  ***************** ALWAYS HAPPY TO SERVE YOU ****************************** \n";
cout<<"\n";
cout<<"\t This restaurant management system was made by AASTU second year students \n"
<<"\t to manage a renowned restaurant called A+ lounge in AASTU";
cout<<"\n\t Contributers";
cout<<"\n\t\t1. Nebyat Bekele\n\t\t2. Rebuma Tadele\n\t\t3. Ruth Asmamaw\n\t\t4. Ruhamma Ashenafi\n\t\t5. Sahib Getu\n\t\t6. Rediat H/Mariam";
...
void deliver(){
 ...
 row=getOrderNumber(code-1000);// getting the index that contains the service code
